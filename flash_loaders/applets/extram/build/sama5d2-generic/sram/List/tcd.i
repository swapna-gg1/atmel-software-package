...
static int _tcd_capture_dma(struct _tcd_desc* desc)
{
	...
	cfg_dma.incr_saddr = 0;
	cfg_dma.incr_daddr = 1;
	cfg_dma.data_width = 2;
	cfg_dma.chunk_size = 0;
...
int tcd_configure_capture(struct _tcd_desc* desc, uint32_t frequency, struct _buffer* buffer)
{
	...
	desc->capture.dma.channel = dma_allocate_channel(tc_id, 0xFF);
	((desc->capture . dma . channel) ? (void) 0 : (__aeabi_assert("desc->capture.dma.channel", "C:\\work\\AtmelSoftPAck\\atmel-software-package-2.17\\drivers\\peripherals\\tcd.c", 209), (__iar_EmptyStepPoint())));
