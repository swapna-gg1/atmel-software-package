...
static struct _dma_channel* pwm_dma_channel = 0;
...
void pwmc_configure_sync_channels(Pwm *pwm, uint32_t mode)
{
	...
	do { if (trace_level >= 5) printf("-D- " "C:\\work\\AtmelSoftPAck\\atmel-software-package-2.17\\drivers\\peripherals\\pwmc.c" ":" "258" " " "pwm: SYNC CHs bitmap 0x%x, Update Mode %u, " "DMA Request Mode %u, Request Comparison Selection %u\n\r", (unsigned)sync_bits, (unsigned)((mode & (0x3u << 16)) >> 16), (unsigned)(0 != (mode & (0x1u << 20))), (unsigned)((mode & (0x7u << 21)) >> 21)); } while (0);
#line 266 "C:\\work\\AtmelSoftPAck\\atmel-software-package-2.17\\drivers\\peripherals\\pwmc.c"
...
void pwmc_set_dma_finished_callback(Pwm *pwm, struct _callback* cb)
{
	...
	if (!pwm_dma_channel) {
		pwm_dma_channel = dma_allocate_channel(0xFF, id);
		((pwm_dma_channel) ? (void) 0 : (__aeabi_assert("pwm_dma_channel", "C:\\work\\AtmelSoftPAck\\atmel-software-package-2.17\\drivers\\peripherals\\pwmc.c", 321), (__iar_EmptyStepPoint())));
	}
...
void pwmc_dma_duty_cycle(Pwm* pwm, uint16_t *duty, uint32_t size)
{
	...
	((pwm_dma_channel) ? (void) 0 : (__aeabi_assert("pwm_dma_channel", "C:\\work\\AtmelSoftPAck\\atmel-software-package-2.17\\drivers\\peripherals\\pwmc.c", 332), (__iar_EmptyStepPoint())));
	memset(&cfg, 0, sizeof(cfg));
	cfg.saddr = (void*)duty;
	cfg.daddr = (void*)&pwm->PWM_DMAR;
	cfg.len = size;
	dma_cfg.incr_saddr = 1;
	dma_cfg.incr_daddr = 0;
	dma_cfg.loop = 0;
	dma_cfg.data_width = 1;
	dma_cfg.chunk_size = 0;
...
void pwmc_configure_external_trigger(Pwm *pwm,
		uint32_t channel, uint32_t value)
{
	do { if (trace_level >= 5) printf("-D- " "C:\\work\\AtmelSoftPAck\\atmel-software-package-2.17\\drivers\\peripherals\\pwmc.c" ":" "529" " " "pwm: external trigger for channel %u, value %08x\n\r", (unsigned)channel, (unsigned)value); } while (0);
	...
	case 2:
		...
		break;
	default:
		((0) ? (void) 0 : (__aeabi_assert("0", "C:\\work\\AtmelSoftPAck\\atmel-software-package-2.17\\drivers\\peripherals\\pwmc.c", 539), (__iar_EmptyStepPoint())));
		break;
	}
}
...
void pwmc_configure_leading_edge_blanking(Pwm *pwm,
		uint32_t channel, uint32_t value)
{
	do { if (trace_level >= 5) printf("-D- " "C:\\work\\AtmelSoftPAck\\atmel-software-package-2.17\\drivers\\peripherals\\pwmc.c" ":" "547" " " "pwm: leading-edge blanking channel %u, value %08x\n\r", (unsigned)channel, (unsigned)value); } while (0);
	...
	case 2:
		...
		break;
	default:
		((0) ? (void) 0 : (__aeabi_assert("0", "C:\\work\\AtmelSoftPAck\\atmel-software-package-2.17\\drivers\\peripherals\\pwmc.c", 557), (__iar_EmptyStepPoint())));
		break;
	}
}
