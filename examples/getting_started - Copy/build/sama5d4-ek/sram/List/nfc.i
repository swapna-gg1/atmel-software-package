...
#line 42 "C:\\work\\AtmelSoftPAck\\atmel-software-package-2.17\\drivers\\nvm\\nand\\nfc.c"
...
#line 43 "C:\\work\\AtmelSoftPAck\\atmel-software-package-2.17\\drivers\\nvm\\nand\\nfc.c"
...
extern void nfc_configure(uint32_t data_size, uint32_t spare_size,
		_Bool read_spare, _Bool write_spare);
...
extern _Bool nfc_sram_scrambling_is_enabled(void);





#line 45 "C:\\work\\AtmelSoftPAck\\atmel-software-package-2.17\\drivers\\nvm\\nand\\nfc.c"
...
#line 46 "C:\\work\\AtmelSoftPAck\\atmel-software-package-2.17\\drivers\\nvm\\nand\\nfc.c"
...
{
	return *(volatile uint32_t*)((0x90000000u));
}

static void _nfc_write_cmd(uint32_t cmd, uint32_t value)
{
	*(volatile uint32_t*)((0x90000000u) + cmd) = value;
}
		...
		_Bool read_spare, _Bool write_spare)
{
	...
	((!read_spare || !write_spare) ? (void) 0 : (__aeabi_assert("!read_spare || !write_spare", "C:\\work\\AtmelSoftPAck\\atmel-software-package-2.17\\drivers\\nvm\\nand\\nfc.c", 75), (__iar_EmptyStepPoint())));

	cfg = (((0x7fu << 24) & (((((spare_size) + (4) - 1) / (4)) - 1) << 24))) |
	      (((0xfu << 16) & ((0xF) << 16))) |
	      (0x7u << 20) |
	      (0x1u << 13);
	...
	default:
		do { if (trace_level >= 1) printf("-F- " "Data size %d unsupported!\r\n", (unsigned)data_size); while (1) ; } while (0);

	}

	((Nfc *)0xFC05C000U)->NFC_CFG = cfg;
}

void nfc_enable(void)
{
	((Nfc *)0xFC05C000U)->NFC_CTRL |= (0x1u << 0);
}

void nfc_disable(void)
{
	((Nfc *)0xFC05C000U)->NFC_CTRL |= (0x1u << 1);
}

uint32_t nfc_get_status(void)
{
	return ((Nfc *)0xFC05C000U)->NFC_SR;
}
...
static _Bool nfc_is_host_busy(void)
{
	return (_nfc_read_status() & (0x1u << 27)) == (0x1u << 27);
}
...
{
	while ((((Nfc *)0xFC05C000U)->NFC_SR & (0x1u << 17)) != (0x1u << 17));
}
...
{
	while ((((Nfc *)0xFC05C000U)->NFC_SR & (0x1u << 16)) != (0x1u << 16));
}
...
{
	while ((((Nfc *)0xFC05C000U)->NFC_SR & (0x1u << 24)) != (0x1u << 24));
}
...
void nfc_send_cmd(uint32_t cmd, uint8_t *cycle_bytes)
{
	...
	switch (cmd & (0x7u << 19)) {
	case (0x5u << 19):
		((Nfc *)0xFC05C000U)->NFC_ADDR = cycle_bytes[0];
		cycle_offset = 1;
		
	case (0x4u << 19):
		nfcdata_addr |= (((0xffu << 24) & ((cycle_bytes[cycle_offset + 3]) << 24)));
		
	case (0x3u << 19):
		nfcdata_addr |= (((0xffu << 16) & ((cycle_bytes[cycle_offset + 2]) << 16)));
		
	case (0x2u << 19):
		nfcdata_addr |= (((0xffu << 8) & ((cycle_bytes[cycle_offset + 1]) << 8)));
		
	case (0x1u << 19):
		nfcdata_addr |= (((0xffu << 0) & ((cycle_bytes[cycle_offset + 0]) << 0)));
		break;
	case (0x0u << 19):
		break;
	default:
		((0) ? (void) 0 : (__aeabi_assert("0", "C:\\work\\AtmelSoftPAck\\atmel-software-package-2.17\\drivers\\nvm\\nand\\nfc.c", 197), (__iar_EmptyStepPoint())));
		break;
	}
...
{
	((Smc *)0xFC05C600U)->SMC_OCMS |= (0x1u << 1);
}
...
_Bool nfc_sram_scrambling_is_enabled(void)
{
	return ((Smc *)0xFC05C600U)->SMC_OCMS & (0x1u << 1);
}
