	...
	uint8_t s_id, uint8_t m_id, uint8_t priority)
{
	if (m_id <= 7) {
		...
		mtx->MATRIX_PR[s_id].MATRIX_PRAS = (mtx->MATRIX_PR[s_id].MATRIX_PRAS &
			(~((0x3u << 0) << m_id))) | (priority << m_id);
	} else {
		((m_id < 16) ? (void) 0 : (__aeabi_assert("m_id < 16", "C:\\work\\AtmelSoftPAck\\atmel-software-package-2.17\\drivers\\peripherals\\matrix.c", 58), (__iar_EmptyStepPoint())));
		m_id = (m_id - 8) << 2;
				  ...
				  uint8_t area_size, uint8_t mask)
{
	...
	((slave_id != 0) ? (void) 0 : (__aeabi_assert("slave_id != 0", "C:\\work\\AtmelSoftPAck\\atmel-software-package-2.17\\drivers\\peripherals\\matrix.c", 94), (__iar_EmptyStepPoint())));
	uint8_t i = mask, j = 0;
	uint32_t value = 0;
	for (i = 1; (i <= mask) && (j < 32 ); i <<= 1, j += 4) {
		if (i & mask)
...
{
	volatile int i;
#line 135 "C:\\work\\AtmelSoftPAck\\atmel-software-package-2.17\\drivers\\peripherals\\matrix.c"
	((Aximx *)0x00700000U)->AXIMX_REMAP = 0;
...
{
	volatile int i;
#line 158 "C:\\work\\AtmelSoftPAck\\atmel-software-package-2.17\\drivers\\peripherals\\matrix.c"
	((Aximx *)0x00700000U)->AXIMX_REMAP = (0x1u << 0);
	...
	cache_clean_region((void*)(0x00200000u), (0x20000u));
	cache_invalidate_region((void*)0, (0x20000u));
}
...
void matrix_set_default_config(void)
{
	...
	for (m_id = 0; m_id < 10; m_id++)
		matrix_configure_master(((Matrix *)0xF001C000U), m_id, (0x4u << 0));

	for (s_id = 0; s_id < 13; s_id++) {
		matrix_configure_slave(((Matrix *)0xF001C000U), s_id, (((0xfu << 18) & ((0) << 18))) |
			(0x0u << 16) | (0x1ffu << 0));
		for (m_id = 0; m_id < 10; m_id++)
			matrix_master_priority_for_slave(((Matrix *)0xF001C000U), s_id, m_id, 0);
		matrix_configure_slave_sec(((Matrix *)0xF001C000U), s_id, 0xFF, 0xFF, 0xFF);
	}

	 
	matrix_remove_write_protection(((Matrix *)0xFC054000U));

	for (m_id = 0; m_id <= 7; m_id++)
		matrix_configure_master(((Matrix *)0xFC054000U), m_id, (0x4u << 0));

	for (s_id = 0; s_id <= 7; s_id++) {
		matrix_configure_slave(((Matrix *)0xFC054000U), s_id, (((0xfu << 18) & ((0) << 18))) |
			(0x0u << 16) | (0x1ffu << 0));
		for (m_id = 0; m_id <= 7; m_id++)
			matrix_master_priority_for_slave(((Matrix *)0xFC054000U), s_id, m_id, 0);
		matrix_configure_slave_sec(((Matrix *)0xFC054000U), s_id, 0xFF, 0xFF, 0xFF);
	}
}
