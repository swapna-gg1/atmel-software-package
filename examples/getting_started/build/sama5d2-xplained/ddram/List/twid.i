...
static struct _async_desc async_desc[(7)];
...
static void _twid_dma_read(struct _twi_desc* desc, struct _buffer* buffer)
{
	...
	if (desc->use_fifo) {
		if ((buffer->size % 4) == 0) {
			desc->addr->TWI_FMR = (desc->addr->TWI_FMR & ~(0x3u << 4)) | (0x2u << 4);
			desc->dma.rx.cfg_dma.data_width = 2;
		} else if ((buffer->size % 2) == 0)  {
			desc->addr->TWI_FMR = (desc->addr->TWI_FMR & ~(0x3u << 4)) | (0x1u << 4);
			desc->dma.rx.cfg_dma.data_width = 1;
		} else {
			desc->addr->TWI_FMR = (desc->addr->TWI_FMR & ~(0x3u << 4)) | (0x0u << 4);
...
static void _twid_dma_write(struct _twi_desc* desc, struct _buffer* buffer)
{
	...
	if (desc->use_fifo) {
		if ((buffer->size % 4) == 0) {
			desc->addr->TWI_FMR = (desc->addr->TWI_FMR & ~(0x3u << 0)) | (0x2u << 0);
			desc->dma.tx.cfg_dma.data_width = 2;
		} else if ((buffer->size % 2) == 0) {
			desc->addr->TWI_FMR = (desc->addr->TWI_FMR & ~(0x3u << 0)) | (0x1u << 0);
			desc->dma.tx.cfg_dma.data_width = 1;
		} else {
			desc->addr->TWI_FMR = (desc->addr->TWI_FMR & ~(0x3u << 0)) | (0x0u << 0);
	...
	if (((status & (0x1u << 1)) == (0x1u << 1))) {
		if (use_fifo) {
			...
			if ((adesc->buf.size - adesc->transferred) >= 4)
				addr->TWI_FMR = (addr->TWI_FMR & ~(0x3u << 4)) | (0x2u << 4);
			else if ((adesc->buf.size - adesc->transferred) >= 2)
				addr->TWI_FMR = (addr->TWI_FMR & ~(0x3u << 4)) | (0x1u << 4);
			else
				addr->TWI_FMR = (addr->TWI_FMR & ~(0x3u << 4)) | (0x0u << 4);
		...
		if (use_fifo) {
			...
			if ((adesc->buf.size - adesc->transferred) >= 4)
				addr->TWI_FMR = (addr->TWI_FMR & ~(0x3u << 0)) | (0x2u << 0);
			else if ((adesc->buf.size - adesc->transferred) >= 2)
				addr->TWI_FMR = (addr->TWI_FMR & ~(0x3u << 0)) | (0x1u << 0);
			else
				addr->TWI_FMR = (addr->TWI_FMR & ~(0x3u << 0)) | (0x0u << 0);
...
{
	uint32_t id = get_twi_id_from_addr(desc->addr);
	((id < (79)) ? (void) 0 : (__aeabi_assert("id < (79)", "C:\\work\\AtmelSoftPAck\\atmel-software-package-2.17\\drivers\\i2c\\twid.c", 774), (__iar_EmptyStepPoint())));
