#line 1 "C:\\work\\AtmelSoftPAck\\atmel-software-package-2.17\\target\\sama5d3\\chip.c"
...
#line 35 "C:\\work\\AtmelSoftPAck\\atmel-software-package-2.17\\target\\sama5d3\\chip.c"
...
#line 37 "C:\\work\\AtmelSoftPAck\\atmel-software-package-2.17\\target\\sama5d3\\chip.c"
...
#line 39 "C:\\work\\AtmelSoftPAck\\atmel-software-package-2.17\\target\\sama5d3\\chip.c"
...
#line 40 "C:\\work\\AtmelSoftPAck\\atmel-software-package-2.17\\target\\sama5d3\\chip.c"
...
static const struct chipid _exid_names[] = {
	{ (0x00444300ul), "SAMA5D31" },
	{ (0x00414300ul), "SAMA5D33" },
	{ (0x00414301ul), "SAMA5D34" },
	{ (0x00584300ul), "SAMA5D35" },
	{ (0x00004301ul), "SAMA5D36" },
};

 
static const struct _console_cfg _console_cfg =
	{ ((Dbgu *)0xFFFFEE00U), 115200, { 1, (1u << 31), 0, (0) }, { 1, (1u << 30), 0, (0) } };
...
static const struct peripheral_dma *get_peripheral_dma(uint32_t id, Dmac *dmac)
{
	...
	if (dmac == ((Dmac *)0xFFFFE600U)) {
		_dmac_peripherals = _dmac0_peripherals;
		count = (sizeof ((_dmac0_peripherals)) / sizeof(*(_dmac0_peripherals)));
	} else if (dmac == ((Dmac *)0xFFFFE800U)) {
		_dmac_peripherals = _dmac1_peripherals;
		count = (sizeof ((_dmac1_peripherals)) / sizeof(*(_dmac1_peripherals)));
	} else {
		return 0;
	}
...
const char* get_chip_name(void)
{
	...
	if ((((Dbgu *)0xFFFFEE00U)->DBGU_CIDR & ~(0x1fu << 0)) ==
			(0x8a5c07c0ul)) {
		uint32_t exid = ((Dbgu *)0xFFFFEE00U)->DBGU_EXID;
		for (i = 0; i < (sizeof ((_exid_names)) / sizeof(*(_exid_names))); i++) {
...
uint32_t get_peripheral_clock_max_freq(uint32_t id)
{
	...
	for (i = 0 ; i < (sizeof ((_peripheral_clock_max_freq)) / sizeof(*(_peripheral_clock_max_freq))) ; i++)
...
uint8_t get_peripheral_dma_channel(uint32_t id, Dmac *dmac, _Bool transmit)
{
...
_Bool is_peripheral_on_dma_controller(uint32_t id, Dmac *dmac)
{
	return get_peripheral_dma(id, dmac) != 0;
}
